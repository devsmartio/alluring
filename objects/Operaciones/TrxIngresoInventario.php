...
/**
 * Description of TrxIngresoInventario
 *
 * @author baci5
 */
class TrxIngresoInventario extends FastTransaction{
    function __construct(){
        parent::__construct();
        $this->setTitle('Ingreso de inventario');
        $this->hasCustomSave = true;
    }
    
    protected function showModule() {
        include VIEWS . "/ingreso_inventario.phtml";
    }
    ...
    public function getProductos(){
        ...
        $marca = Collection::get($this->db, 'marca')->select(['nombre', 'id_marca'], true);
        ...
        for($i=0; count($productos) > $i; $i++){
            ...
            $marcaProd = $marca->where(['id_marca' => $productos[$i]['id_marca']])->single();
            $productos[$i]['nombre_marca'] = $marcaProd['nombre']; 
